...
class Universal extends CI_Model {
  ...
  */
  public function get($distinct=false,$table="",$select="*",$result="all",$where=array(),$like=array(),$join=array(),$limit=array(),$order = array(),$group = array(),$jointype = "left", $or_like=array()){
    $this->db->reset_query();
    if($distinct){
      $this->db->distinct();
    }
  ...
  */
   ...
   */
    ...
    */
    ...
    public function datatables2($table = "", $select = "",  $where = array(), $join = array(), $limit = array(), $order = array(), $like = array(), $distinct = false, $group = array()){
     $this->db->select($select);
     $this->db->from($table);
     if(!empty($where)){
       foreach ($where as $key => $value) {
